...
from .tools import SearchImages
...
def my_writer_before_agent_callback(callback_context: CallbackContext) -> None:
    ...
    refine_document: str = callback_context.state.get("summary_document")
    if not refine_document:
        raise ValueError("refine_document is missing in session state. Please ensure RefineAgent ran successfully.")
    ...
    callback_context.state["research_doc"] = refine_document
